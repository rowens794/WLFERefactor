...
class WinnersCircle extends Component {
  ...
  calculateWinners(players, competition) {
    ...
    switch (competition.InterimPrize) {
      case '52W':
        interimPrizeAmt = totalPrizePool * 0.05;
        prizeAwardFreq = 14;
        break;
      case '54W':
        interimPrizeAmt = totalPrizePool * 0.05;
        ...
        break;
      case '104W':
        interimPrizeAmt = totalPrizePool * 0.1;
        prizeAwardFreq = 28;
        break;
      default:
        interimPrizeAmt = 0;
    ...
    switch (competition.Payout) {
      case '1':
        ...
        break;
      case '2':
        firstPrize = (totalPrizePool - numberOfIterimPrizes * interimPrizeAmt) * 0.75;
        secondPrize = (totalPrizePool - numberOfIterimPrizes * interimPrizeAmt) * 0.25;
        break;
      case '3':
        firstPrize = (totalPrizePool - numberOfIterimPrizes * interimPrizeAmt) * 0.6;
        secondPrize = (totalPrizePool - numberOfIterimPrizes * interimPrizeAmt) * 0.25;
        thirdPrize = (totalPrizePool - numberOfIterimPrizes * interimPrizeAmt) * 0.15;
        break;
      default:
        firstPrize = null;
        secondPrize = null;
    ...
    if (prizeAwardFreq) {
      //test if interim prizes are actually awarded
      for (let j = prizeAwardFreq; j < competitionLength; j += prizeAwardFreq) {
        ...
        let periodEndDate = moment(new Date(competition.StartDate))
          .add(j, 'days')
          .format('M/D/YYYY');
        let sortedPlayers = this.calculateWinnerForPeriod(
          this.state.playerData,
          periodEndDate,
          moment(new Date(periodEndDate))
            .subtract(prizeAwardFreq, 'days')
            .format('M/D/YYYY'),
        );
    ...
    if (firstPrize) {
      //test if interim prizes are actually awarded
      let competitionEndDate = moment(new Date(competition.StartDate))
        .add(competitionLength, 'days')
        .format('M/D/YYYY');
      let sortedPlayers = this.calculateWinnerForPeriod(
        this.state.playerData,
        competitionEndDate,
        moment(new Date(competition.StartDate)).format('M/D/YYYY'),
      );
    ...
    if (secondPrize) {
      //test if second prize is actually awarded
      let competitionEndDate = moment(new Date(competition.StartDate))
        .add(competitionLength, 'days')
        .format('M/D/YYYY');
      let sortedPlayers = this.calculateWinnerForPeriod(
    ...
    if (thirdPrize) {
      //test if third prize is actually awarded
      let competitionEndDate = moment(new Date(competition.StartDate))
        .add(competitionLength, 'days')
        .format('M/D/YYYY');
      let sortedPlayers = this.calculateWinnerForPeriod(
