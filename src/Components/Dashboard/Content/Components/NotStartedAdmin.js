...
class NotStartedAdmin extends Component {
  ...
  saveCompetition() {
    ...
    if (!fieldErrors) {
      axios
        .post(Config.backendRootURL + '/addUserToComp', {
    ...
    return (
      <div>
        <div className={css(styles.primaryText)}>
          The competition has not yet begun. Be sure to sign back in on or as close to {this.state.startDate} as
          possible to log your starting weight.
        </div>
        <br />
        ...
        <Container className={css(styles.addUserForm)}>
          <form>
            <Row>
              <Col
                className={css(styles.addPlayerSection)}
                xs={{ size: 12, offset: 0 }}
                sm={{ size: 10, offset: 1 }}
                md={{ size: 6, offset: 3 }}
                lg={{ size: 6, offset: 3 }}
              >
                <Row>
                  As the creator of {this.state.competitionName}, you can add new players to the competition prior to{' '}
                  {this.state.startDate}.
                </Row>

                <Row className={css(styles.resMessage)}>
                  <p className={css(styles.resMessage)} id="resMessage" />
                </Row>

                <Row className={css(styles.playerList)}>
                  <input id="name" placeholder="particpant name" className={css(styles.inputField)} />
                </Row>
                <Row>
                  <input id="email" placeholder="participant email" className={css(styles.inputField)} />
                </Row>
