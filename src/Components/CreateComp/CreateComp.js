...
class CreateComp extends Component {
  ...
  async componentWillMount() {
    if (!localStorage.getItem('accountVerified')) {
  ...
  saveCompetition() {
    ...
    if (CompetitionName === '') {
      ...
      errorObject.errorMessage = 'The competition must have a name.';
      fieldErrors = true;
    }
    if (StartDate === '') {
      errorObject.errorStartDate = true;
      errorObject.errorMessage = 'Please select a start date (and it must be no earlier than today).';
      fieldErrors = true;
    }
    if (EntryFee === '' || isNaN(EntryFee)) {
      errorObject.errorEntryFee = true;
      errorObject.errorMessage =
        'You must establish an entry fee and the entry fee must be a number (it can be 0 though).';
      fieldErrors = true;
    }
    if (Payout === '') {
      errorObject.errorPayout = true;
      errorObject.errorMessage = 'Select a payout structure.';
      fieldErrors = true;
    }
    if (InterimPrize === '') {
      errorObject.errorCompLength = true;
      errorObject.errorMessage = 'Select an interim prize or "None"';
      fieldErrors = true;
    }
    if (Players.length === 0) {
      errorObject.errorParticipants = true;
      errorObject.errorMessage = 'Atleast 1 other player must be invited to the competition.';
      fieldErrors = true;
    }
    ...
    if (!fieldErrors) {
      axios
        .post(Config.backendRootURL + '/createCompetition', {
          token: localStorage.getItem('userToken'),
          competitionInfo: saveObject,
        })
        .then(function(response) {
          if (response.data === '{"login":"failed"}') {
          } else if (response.data.status === 'duplicateEmails') {
            self.setState({
              errorMessage: 'You may only invite 1 participant per email address',
            });
          } else {
                  ...
                  <Col>
                    <div className={css(styles.itemContainer)}>
                      <label className={css(styles.inputLabel)}>
                        Competition Name:
                        <input id="compName" type="text" className={css(styles.inputBox)} />
                      </label>
                    </div>
                    ...
                    <div className={css(styles.itemContainer)}>
                      <label className={css(styles.inputLabel)}>
                        Entry Fee:
                        <input id="entryFee" type="text" className={css(styles.inputBox)} />
                      </label>
                    </div>
                ...
                <Row>
                  <Col className={css(styles.addPlayerSection)}>
                    ...
                    <Row className={css(styles.playerList)}>
                      ...
                      <Col>
                        <p className={css(styles.addPlayerButton)} onClick={() => this.addNewParticipant()}>
                          add participant
                        </p>
                      </Col>
                    </Row>
                    ...
                    {this.state.participants.map((participant, index) => (
                      <div key={index}>
                        <Row className={css(styles.playerList)}>
                          ...
                          <Col>
                            <p className={css(styles.deletePlayer)} onClick={() => this.deleteParticipant(index)}>
                              delete participant
                            </p>
                          </Col>
                        </Row>
