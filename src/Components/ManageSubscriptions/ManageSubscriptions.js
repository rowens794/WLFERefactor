...
class ManageSubscriptions extends Component {
  ...
  updateEmailPermissions() {
    ...
    axios
      .post(Config.backendRootURL + '/changeemailpref', {
        token: localStorage.getItem('userToken'), //fetch the JWT from local storage
      })
      .then(function(response) {
        if (response.data.status === 'failed') {
          ...
          <div>
            <Container>
              <Row>
                ...
                >
                  <p>
                    Turning off emails will disable all weekly competition updates and reminders. You may be susceptible
                    to missing a weigh-in if you disable emails.
                  </p>
                  <br />
